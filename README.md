...
The micro:bit uses a specialized approach to enable Bluetooth functionality:

### How Bluetooth Works in This Project:

The password generator uses a two-step approach for Bluetooth communication:

1. **Radio-to-BLE Bridge:**
   - We use the micro:bit's radio module in our code
   - A special firmware creates a bridge between the radio module and Bluetooth
   - This allows our code to use simple radio commands while still enabling Bluetooth connectivity

2. **Special Firmware Deployment:**
   - The first time you set up, run: `./scripts/get_ble_firmware.sh`
   - This downloads the special bridge firmware
   - Then deploy normally with `./scripts/deploy.sh`

### Setting Up BLE Connectivity:

1. **Installing the Bridge Firmware:**
   ```bash
   # Download the special BLE bridge firmware
   ./scripts/get_ble_firmware.sh
   
   # Deploy your password generator with BLE support
   ./scripts/deploy.sh
   ```

2. **Pairing with Your Devices:**
   - After deploying the code, press and hold buttons A+B for 3+ seconds
   - The diamond pattern indicates pairing mode is active
   - On your phone or computer, scan for "BBC micro:bit" in Bluetooth settings

3. **Compatible Apps for Receiving Passwords:**
   - **Micro:bit Blue** (recommended for this project)
   - **nRF Connect** with the UART service
   - Any app that supports BLE UART services
